...
- `ADMIN_POOL_REQUEST_RATE` / `ADMIN_POOL_REQUEST_RATE_MAX`（20 / 120）—— **纯请求速率**限流（`14003`）的冷却基数与封顶。远小于上一行是有意的：见 [10.4.1](#1041-事故复盘一次-14003-抖动如何变成-10-分钟全站-503)
...
| 码 | 名称 | 本网关处置 | 为什么 |
|----|------|-----------|--------|
| `14003` | **RateLimitError** | `soft_rate`，但走**秒级**冷却档 | 纯请求速率限流（`quota_request_limit`），不是额度；上游文案即「请稍后重试」 |
...
| 步 | 事实 |
|----|------|
| 1 | 上游返回 `14003 RateLimitError`：`{"code":14003,"msg":"too many requests","displayMsg":{"zh":"请求过于频繁，请稍后重试。"}}` |
| 2 | 旧代码里 `14003` **没有任何专门处理**，落到通用 `"too many requests"` 文案分支 → `soft_rate` |
...
所以那句「全部禁用或额度耗尽」是**误判**：真实情况只是「大家刚被限流了几十秒」。
`soft_rate` 的文档写的是「**秒级**有界退避」，而配置值是 600 秒 —— 代码与自己的
注释矛盾了整整一个数量级，这是本次事故的根。

**三处修复**：

1. **`14003` 单独走秒级冷却档**（`ADMIN_POOL_REQUEST_RATE` / `_MAX`，默认 20/120 秒）。
   它和 `6000`–`6008` 那类「每分钟/每小时配额」量级不同：官方 UI 对它的建议是
   「请切换模型或稍后重试」，不是「等 10 分钟」。
...
> `MAX_ROTATE` 次上游调用。若上游限流是**按出口 IP**（而非按账号）计的，那么
> 换号根本无解，继续轮转只会加重风控。`admin/pool.py` 里已有 `WafIpGate`
> 这个「60s 内 N 个**不同账号**命中即判定 IP 级拦截、立刻停止轮转」的先例，
> 把限流也接进同一个闸门是下一步该做的事。
